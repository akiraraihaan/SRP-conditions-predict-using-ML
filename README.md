...
| Compute | CPU | GPU (free Colab) for training; CPU (Raspberry Pi) for the edge benchmark |
...
notebooks/        colab_runner.ipynb — thin; no experiment logic. The primary runner.
                  kaggle_runner.ipynb — same shape, for the alternative platform
docs/             COLAB_SETUP.md, KAGGLE_SETUP.md
...
Everything except `07` runs on free Colab. `07` runs on the Pi, and on none of
the GPU platforms — it refuses to run when CUDA is visible.
...
See [docs/COLAB_SETUP.md](docs/COLAB_SETUP.md) for the Drive layout, uploading the
images and wiring `DATA_ROOT` to them. [docs/KAGGLE_SETUP.md](docs/KAGGLE_SETUP.md)
covers the same ground for Kaggle, which needs account verification before its GPU
and Internet are available. HANDOVER.md §4.0 has the platform comparison.
...
- [x] (f) remaining scripts, aggregate, Colab and Kaggle runners, docs
